...
if (typeof Xenia!=='undefined') {
    $.extend(XeniaOptions, Xenia);
}
...
var XeniaTheme = {
    ...
    init:function() {
        XeniaTheme.intro();
        XeniaTheme.navigation();
        XeniaTheme.portfolio();
        XeniaTheme.shortcodes();
        XeniaTheme.animations();
        XeniaTheme.loader();
        XeniaTheme.contact();
        XeniaTheme.map();
        XeniaTheme.tweaks();
        XeniaTheme.parallax();
        XeniaTheme.videos();
        XeniaTheme.imageSlider();
        XeniaTheme.contentSlider();
        XeniaTheme.blog();
        XeniaTheme.twitter();
    },
    ...
    loader:function() {
        if (XeniaOptions.loader) {
            ...
            if ($('#intro').attr('data-type')=='video' && !Modernizr.touch) {
                $(window).on('xenia.intro-video', function() {
    ...
    navigation:function() {
        ...
        if ($('#intro').length>0 && XeniaOptions.navigation=='sticky') {
		...
		//Scroll to anchor links
       	$('a[href^=#]').click(function(e) {
            if ($(this).attr('href')!='#' && !$(e.target).parent().parent().is('.navbar-nav') && !$(this).attr('data-toggle')) {
                $(document).scrollTo($(this).attr('href'), XeniaOptions.scrollSpeed, {offset:{top:-85, left:0}});
                e.preventDefault();
            }
       	});
        ...
        $(document).ready(function() {
            $('.navbar-nav').onePageNav({
                currentClass:'active',
                changeHash:false,
                scrollSpeed:XeniaOptions.scrollSpeed,
                scrollOffset:85,
                scrollThreshold:0.5,
                filter:'li a[href^=#]',
                begin:function() {
        ...
        if (document.location.hash && XeniaOptions.loader) {
    ...
    intro:function() {
        ...
        if ($that.attr('data-type')=='static-image' || $that.attr('data-type')=='image-pattern') {
            ...
            if ($elements.length>0) {
                ...
                $(window).on('xenia.complete', function() {
                    $($elements).each(function(i) {
                        var $this = $(this);
                        setTimeout(function() {
                            $this.addClass('complete');
                        }, 0+(i*XeniaOptions.intro.animateDelay));
                   });
               });
            } else {
                verticalAlignContent();
        ...
        //Slideshow
            ...
            var contentListShow = function($that, $contentList, index) {
                ...
                if ($elements.length>0) {
                    ...
                    $($elements).each(function(i) {
                        ...
                        setTimeout(function() {
                            $this.addClass('complete');
                        }, XeniaOptions.intro.animateDelayFirst+(i*XeniaOptions.intro.animateDelay));
                   });
                } else {
                    $current.show();
                    verticalAlignContent();
            ...
            var tempInt = XeniaOptions.intro.animateDelayFirst;
            ...
            var slideshowChange = function($that, index) {
                ...
                if ($contentList || slideshowIsFirst) {
                    contentListHide($that, $contentList, function() {
                        if (!slideshowIsFirst) {
                            if (XeniaOptions.intro.animateDelayFirst==0) XeniaOptions.intro.animateDelayFirst = tempInt;
                            contentListShow($that, $contentList, index);
                        } else {
                            $(window).on('xenia.complete', function() {
                                XeniaOptions.intro.animateDelayFirst = 0;
                                contentListShow($that, $contentList, index);
                           });
                        }
                   });
            ...
            var slideshowCreate = function() {
                ...
                $(window).on('xenia.loaded', function() {
                ...
                $(document).ready(function() {
                    ...
                    $(window).on('YTAPIReady', function() {
                        reserveTimer = setTimeout(function() {
                            $(window).trigger('xenia.intro-video');
                            onlyForFirst = false;
                        }, 5000);
                   	});
    ...
    portfolio:function() {
                    ...
                    $.get($that.attr('data-url')).done(function(response) {
						...
						$container.imagesLoaded(function() {
							...
							$loading.animate({opacity:0}, {duration:200, queue:true, complete:function() {
                                ...
                                XeniaTheme.imageSlider($container, function() {
        ...
        $('a[href^="#view-"]').click(function() {
            ...
            if ($item.length>0) {
                $(document).scrollTo('#portfolio', XeniaOptions.scrollSpeed, {offset:{top:-85, left:0}, onAfter:function() {
                    $item.trigger('click');
                }});
            }
       });
        ...
        $(window).load(function() {
            $('.parallax').each(function() {
                if ($(this).attr('data-image')) {
                    ...
                    if (XeniaOptions.parallax && !Modernizr.touch && !/MSIE/.test(navigator.userAgent)) {
    ...
    map:function() {
        ...
        $(window).load(function() {
            ...
            var companyImage = new google.maps.MarkerImage('layout/images/marker.png', null, null, new google.maps.Point(27, 62), new google.maps.Size(54, 64));
    ...
    twitter:function() {
        ...
        $(window).load(function() {
            $('.twitter-feed').bind('loaded', function() {
                ...
                XeniaTheme.contentSlider($(this), 'li');
           }).tweet({
    ...
    animations:function() {
        if (Modernizr.touch) {
            XeniaOptions.animations = false;
        }
		
        if (!XeniaOptions.animations) {
...
function shareTo(network, title, image, url) {
    return XeniaTheme.share(network, title, image, url);
}
